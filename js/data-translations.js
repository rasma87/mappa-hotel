...
function renderHotelInfo(lang) {
    ...
    el.innerHTML = `
        ...
        <div style="display:flex;justify-content:center;gap:8px;padding:0 0 20px">
            ${ ['it','en','fr','es'].map(l => `<button onclick="setLang('${l}')" class="lang-btn${l===lang?' active':''}" style="touch-action:manipulation">${l.toUpperCase()}</button>`).join('') }
        </div>
